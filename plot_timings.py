...
def plot_timings(csv_file='timings.csv'):
    ...
    pivot_df = df.pivot(index='poly_size', columns='function_name', values='time_seconds')
    ...
    plt.xticks(x + width/2, pivot_df.index)
    plt.legend()
    plt.yscale('log')
    ...
    # 4. Speedup ratio
    if 'naive' in pivot_df.columns and 'karatsuba' in pivot_df.columns:
        plt.figure(figsize=(10, 6))
        speedup = pivot_df['naive'] / pivot_df['karatsuba']
        plt.plot(pivot_df.index, speedup, marker='o', color='green', linewidth=2)
        plt.axhline(y=1, color='red', linestyle='--', alpha=0.7, label='No speedup')
        plt.xlabel('Polynomial Size')
        plt.ylabel('Speedup (Naive/Karatsuba)')
        plt.title('Karatsuba Speedup over Naive')
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.savefig('plots/speedup_ratio.png', dpi=300, bbox_inches='tight')
        plt.close()
